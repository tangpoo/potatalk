...
public class ChatRoomService {
    ...
    public Mono<ChatRoom> createChatRoom(CreateChatRoomDto createChatRoomDto) {
        ...
        return chatRoomRepository
                .save(ChatRoom.create(createChatRoomDto, ChatRoomStatus.GROUP))
                .flatMap(
                        chatRoom ->
                ...
                .flatMap(
                        chatRoom ->
                                chatRoomPublisher
                                        .sendAddTopicEvent("roomId-" + chatRoom.getId().toString())
                                        .thenReturn(chatRoom))
    ...
    public Mono<ChatRoom> createOneToOneChatRoom(CreateChatRoomDto createChatRoomDto) {
        ...
        return chatRoomRepository
                .findOneToOneChatRoom(memberId, friendId, ChatRoomStatus.ONE_TO_ONE)
                .switchIfEmpty(
                        Mono.defer(
                                () -> {
                                    ChatRoom chatRoom =
                                            ChatRoom.create(
                                                    createChatRoomDto, ChatRoomStatus.ONE_TO_ONE);
                                    return chatRoomRepository
                                            .save(chatRoom)
                                            .flatMap(
                                                    savedRoom -> {
                                                        Participation memberParticipation =
                                                                Participation.create(
                                                                        ...
                                                                        savedRoom.getId(),
                                                                        ParticipationStatus.JOINED);
                                                        Participation friendParticipation =
                                                                Participation.create(
                                                                        friendId,
                                                                        savedRoom.getId(),
                                                                        ParticipationStatus.JOINED);
    ...
    private Mono<ChatRoom> checkMemberParticipation(ChatRoom chatRoom, Long memberId) {
        return participationRepository
                .findByRoomIdAndMemberId(chatRoom.getId(), memberId)
    ...
    private Mono<ChatRoom> handleExistingParticipation(
            final Participation participation, final ChatRoom chatRoom) {
    ...
    public Flux<Participation> findAllInviteParticipation(final Long memberId) {
        return participationRepository.findAllByParticipationStatusIsInvited(
                memberId, ParticipationStatus.INVITED);
    }
                ...
                .flatMap(
                        participation -> {
                            if (!participation
                                    ...
                                    .equals(ParticipationStatus.INVITED)) {
    ...
    public Mono<ChatRoomInfoRes> findChatRoomInfo(Long roomId) {
        return chatRoomRepository
                ...
                .flatMap(
                        chatRoom ->
                                participationRepository
                                        .findAllIdByRoomId(roomId)
                                        .collectList()
                                        .map(
                                                participationIds ->
                                                        ChatRoomInfoRes.from(
                                                                chatRoom, participationIds)));
    }
