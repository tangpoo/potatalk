...
public class RedisTopicManager {
    ...
    public Mono<Void> addTopicForChatRoom(String chatRoomId) {
        ...
        return redisTemplate
            ...
            .doOnSuccess(result -> log.info("Redis channel registered for: " + newTopic.getTopic()))
        ...
        listenerContainer
            ...
            .doOnNext(message -> {
                ...
                messageListenerAdapter.onMessage(
                    new DefaultMessage(topic.getTopic().getBytes(), message.getBytes()),
                    null
                );
            })
            .doOnError(e -> log.error("Error while receiving Redis message", e))
            .doOnComplete(() -> log.info("Completed receiving message from Redis: " + topic.getTopic()))
            .subscribe();  // 구독 시작
    }
