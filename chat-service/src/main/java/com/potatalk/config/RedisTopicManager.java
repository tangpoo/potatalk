    ...
    public Mono<Void> addTopicForChatRoom(String chatRoomId) {
        ...
        return Mono.fromRunnable(() -> {
            if (!topics.containsKey(chatRoomId)) {
                ...
                Flux<String> messageFlux = listenerContainer.receive(newTopic)
                    .map(message -> (String) message.getMessage())
                    .doOnSubscribe(s -> log.info("Subscribed to topic: " + newTopic.getTopic()))
                    .doOnNext(message -> log.info("Received message from Redis: " + message))
                    .doOnError(e -> log.error("Error while receiving Redis message", e));

                Disposable subscription = messageFlux.subscribe(message -> {
                    log.info("Received message from Redis: " + message);

                    // Redis 채널 이름을 가져와 DefaultMessage에 전달
                    ChannelTopic topic = topics.get(chatRoomId);
                    if (topic != null) {
                        byte[] channel = topic.getTopic().getBytes();  // 채널을 topic에서 가져옴
                        Message redisMessage = new DefaultMessage(channel, message.getBytes());
                        messageListenerAdapter.onMessage(redisMessage, null);
                    } else {
                        log.error("Topic not found for chatRoomId: " + chatRoomId);
                    }
                });
